...
*Existing generators
         egu                             existing generators
                 ...
                 hydroegu(egu) CHANGED FROM RENEWEGU TO EGU ON JAN 4 2023 BECAUSE WANT HYDRO TO COUNT TOWARDS PRM IF STILL IN FLEET
...
*CAPACITY AND CAPACITY DERATING PARAMETERS 
         ...
         pCapacderate(egu,h)       fractional derating of capacity 
...
*ZONAL PROPERTIES
        ...
        pLinesource(l)                                  zone that is the source of line l
                ...
                pTransEff
                pLinecapacderate(l,h)           fractional thermal derating of line capacity 
...
$load pCapac, pCapacderate, pHr, pOpcost, pRamprate, pCO2emrate, pCO2cost
$load pMaxgensolar, pMaxgenwind
$load pStoinenergymarket,pEfficiency,pMaxsoc,pMinsoc,pCapaccharge
$load pGenzone, pDemand, pLinesource, pLinesink, pLinecapac, pLinecapacderate, pTransEff
...
limitallresup(genegu,h) .. vGen(genegu,h) + vFlex(genegu,h) + vCont(genegu,h) + vRegup(genegu,h) =l= pCapac(genegu) * (1-pCapacderate(genegu,h));
