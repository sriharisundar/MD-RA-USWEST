...
def addMaxNewBuilds(db,df,thermalSet,stoTechSet,dacsSet,CCSSet,zoneSet,maxCapPerTech,maxREPerZone,maxREInInterconn,mwToGW):
    ...
    for pt in ['Wind','Solar']:
        ...
        add0dParam(db,'pNMaxTotal'+pt,maxREInInterconn[pt]/genCaps.mean())
        #Each zone within interconnection/entire region
        add1dParam(db,((pd.Series(maxREPerZone[pt]))/genCaps.mean()).to_dict(),zoneSet,[k for k in maxREPerZone[pt]],'pNMaxRegional'+pt)   
        #Grid cell
    ...
    #Storage per zone. Use positive continuous variable for added power & energy separately,
...
def addWSMinGen(db, wsGenFracOfDemand):
    add0dParam(db,'pWSGenAsFracOfDemand',wsGenFracOfDemand/100)

##### ADD NEW LINE PARAMETERS
def addNewLineParams(db, lineDists, lineCosts, lineSet, maxCapPerTech, zoneOrder, interconn, mwToGW, lineLife=60):
