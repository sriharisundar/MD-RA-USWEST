...
def getNewTechs(regElig,regCostFrac,currYear,stoInCE,seasStoInCE,fuelPrices,yearIncDACS,
                transRegions,contFlexInelig,weatherYears,onlyNSPSUnits=True,allowCoalWithoutCCS=False,
...
def extractATBDataForCurrentYear(newTechsCE,newPlantDataDir,currYear,scenario='Moderate'):
    ...
    # print('USING PV + STORAGE INSTEAD OF JUST PV FOR SOLAR PV - SEE IMPORTNEWTECHS.PY. To fix, delete next two lines & uncomment following 2 lines. This is for CESM testing.')
    # ptToATBTechAlias = {'Solar PV':'Utility-Scale PV-Plus-Battery','Wind':'Land-Based Wind','Battery Storage':'Utility-Scale Battery Storage','Nuclear':'Nuclear',
...
#Units: CAPEX(2012$/MW), FOM(2012$/MW/yr), VOM(2012$/MWh), Efficiency, ECAPEX(2012$/MWH).
...
def addHydrogenCostAndEffParams(newTechsCE,pcapex=1058000,fom=0,vom=0,eff=.49,ecapex=160):
    newTechsCE.loc[newTechsCE['PlantType'] == 'Hydrogen','CAPEX(2012$/MW)'] = pcapex
    newTechsCE.loc[newTechsCE['PlantType'] == 'Hydrogen','FOM(2012$/MW/yr)'] = fom
    newTechsCE.loc[newTechsCE['PlantType'] == 'Hydrogen','VOM(2012$/MWh)'] = vom
    newTechsCE.loc[newTechsCE['PlantType'] == 'Hydrogen','Efficiency'] = eff
    newTechsCE.loc[newTechsCE['PlantType'] == 'Hydrogen','ECAPEX(2012$/MWH)'] = ecapex
    return newTechsCE

def scaleUpCapitalCostsForMultiYears(newTechsCE,weatherYears):
    numYearsInCE = len(weatherYears)
    newTechsCE['CAPEX(2012$/MW)'] *= numYearsInCE
    newTechsCE['FOM(2012$/MW/yr)'] *= numYearsInCE
    newTechsCE['ECAPEX(2012$/MWH)'] *= numYearsInCE
    return newTechsCE
